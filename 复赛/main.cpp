...
//手工栈------------------------------------------------------------------------------------------------------------------------
typedef pair<uint32_t, uint32_t> id_money;
vector<uint32_t> input_data;
vector<uint32_t> money;
...
{
	uint32_t res;
	uint32_t num_front = 0, num_back = 0;
	while (num_front < len)
	{
		//-----------------------------------------------
		res = 0;
...
bool file_input(const char* File_dir)
{
	...
	uint32_t father_id, child_id, moneys;
	FILE* file = fopen(File_dir, "r");
	if (file)
	{
		while (fscanf(file, "%u,%u,%u", &father_id, &child_id, &moneys) != EOF)
		{
...
vector<vector<uint32_t>> ans3, ans4, ans5, ans6, ans7;
vector<uint32_t> temp_ans3(3), temp_ans4(4), temp_ans5(5), temp_ans6(6), temp_ans7(7);
vector<bool> vis;
vector<bool> vis_inv;
...
uint32_t money_stack[10];
void dfs_forward(const uint32_t& target_id, uint32_t cur_id, uint32_t moneys)
{
	bool inLimit = false;
